...
- User shortcut: when I say `cp`, treat it as a one-shot request to commit and push the relevant current changes exactly once. The shortcut expires immediately after that commit/push attempt. Never carry a prior `cp` forward to later user messages; every commit/push requires a fresh explicit `cp` or equivalent instruction.
